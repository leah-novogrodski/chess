...
Sections parseSections(const std::string &text)
{
    ...
    while (std::getline(stream, line))
    {
        ...
        if (t == io_format::SECTION_BOARD)
        {
            where = BOARD;
            continue;
        }
        if (t == io_format::SECTION_COMMANDS)
        {
            where = COMMANDS;
            continue;
        }
        ...
        if (where == BOARD)
            ...
            throw BoardError("UNRECOGNIZED_SECTION_LINE");
    }
    return s;
}
...
Board parseBoard(const std::vector<std::string> &boardLines)
{
    ...
    for (const auto &line : boardLines)
        rawGrid.push_back(splitWords(line));
...
bool isValidToken(const std::string &t)
{
    ...
    if (t.size() != 2)
        return false;
    if (t[0] != COLOR_WHITE_CHAR && t[0] != COLOR_BLACK_CHAR)
        return false;
