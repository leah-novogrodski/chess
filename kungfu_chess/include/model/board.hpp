...
public:
    ...
    void add_piece(const Position &pos, const chess::Piece &piece);
    ...
    std::optional<chess::Piece> get_piece(const Position &pos) const;

private:
    int m_width;
    int m_height;
    std::vector<std::optional<chess::Piece>> m_grid;
